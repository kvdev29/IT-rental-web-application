...
class ChangePasswordForm(FlaskForm):
    ...
    new_password = PasswordField('New Password', validators=[DataRequired(), password_strength])
...
class AssetForm(FlaskForm):
    name = StringField('Asset Name', validators=[DataRequired(), Length(min=2, max=120), no_html])
    ...
    manufacturer = StringField('Manufacturer', validators=[Optional(), Length(max=80), no_html])
    model = StringField('Model', validators=[Optional(), Length(max=80), no_html])
    serial_number = StringField('Serial Number', validators=[Optional(), Length(max=100), no_html])
    status = SelectField('Status', choices=[])
    location = StringField('Location', validators=[Optional(), Length(max=100), no_html])
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.assigned_to_id.choices = [(0, '-- Unassigned --')] + [(u.id, u.full_name) for u in users]


class TicketForm(FlaskForm):
    title = StringField('Summary', validators=[DataRequired(), Length(min=5, max=200), no_html])
    description = TextAreaField('Description', validators=[DataRequired(), Length(min=10, max=5000), no_html])
    priority = SelectField('Priority', choices=[])
    category = SelectField('Category', choices=[])
    related_asset_id = SelectField('Related Asset (optional)', coerce=int, validators=[Optional()])
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.related_asset_id.choices = [(0, '-- None --')] + [(a.id, f'{a.asset_tag} - {a.name}') for a in assets]


class AdminTicketForm(TicketForm):
    status = SelectField('Status', choices=[])
    assigned_to_id = SelectField('Assign To', coerce=int, validators=[Optional()])
    resolution_notes = TextAreaField('Resolution Notes', validators=[Optional(), Length(max=5000), no_html])
...
class CommentForm(FlaskForm):
    body = TextAreaField('Comment', validators=[DataRequired(), Length(min=1, max=2000), no_html])
    is_internal = BooleanField('Internal note (admin-only)')
    submit = SubmitField('Post Comment')


class AdminEditUserForm(FlaskForm):
    full_name = StringField('Full Name', validators=[DataRequired(), Length(min=2, max=120), no_html])
    email = StringField('Email', validators=[DataRequired(), Email(), Length(max=120)])
    department = StringField('Department', validators=[Optional(), Length(max=80), no_html])
    role = SelectField('Role', choices=[('user', 'Standard User'), ('admin', 'Administrator')])
